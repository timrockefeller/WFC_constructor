...
public class ModulePrototype : MonoBehaviour {
	...
	static void DrawGizmo(ModulePrototype modulePrototype, GizmoType gizmoType) {
			...
			for (int i = 0; i < 6; i++) {
				...
				if (hint.Mesh != null) {
					Gizmos.DrawMesh(hint.Mesh,
						position + rotation *  Orientations.Direction[i] * AbstractMap.BLOCK_SIZE,
		...
		for (int i = 0; i < 6; i++) {
			...
			Handles.Label(position + rotation * Orientations.Rotations[i] * Vector3.forward * AbstractMap.BLOCK_SIZE / 2f, face.ToString(), ModulePrototype.style);
		}
	}
#endif
